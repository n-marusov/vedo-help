...
main() {
    ...
    for ep in "backend:${BACKEND_PORT}/health" "chroma:8000/api/v1/heartbeat"; do
        ...
        if curl -f -s -m 5 "http://${url}" > /dev/null 2>&1; then
            log_success "${svc} (http://${url})"
        else
            log_warn "${svc} (http://${url}) — not accessible"
        fi
    done
    echo ""
    ...
    if [ "$DETAILED" = true ]; then
        ...
        dc logs --tail=100 --no-color 2>/dev/null | grep -i "error\|fatal\|panic\|exception" | tail -5 || echo "  No recent errors found"
        echo ""
    fi
