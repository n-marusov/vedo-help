...
6. [E2E-тесты (Playwright)](#e2e-тесты-playwright)
    1. [API-тесты (без браузера)](#api-тесты-без-браузера)
...
| Файл | Что проверяет |
|------|---------------|
| `api-backend.spec.ts` | Backend API напрямую: 37 тестов, 6 групп эндпоинтов |

### API-тесты (без браузера)

Файл `api-backend.spec.ts` содержит 37 тестов, которые обращаются к backend API напрямую через `APIRequestContext` Playwright — без открытия браузера. Это быстрая проверка бэкенд-логики без UI.

Тесты покрывают шесть групп эндпоинтов:

| Группа | Тесты | Что проверяют |
|--------|-------|---------------|
| **Health** | TC-API-001 | `GET /health` — публичный эндпоинт |
| **Auth** | TC-API-002–006 | `/api/auth/me` и `/api/auth/logout` — валидный/невалидный/без токена |
| **Collections** | TC-API-010–015 | CRUD коллекций, 404 для несуществующей, 401 без auth |
| **Documents** | TC-API-020–029 | Загрузка, список, soft-delete, batch-delete, ZIP, 422/400/404 |
| **Query/SSE** | TC-API-030–032 | SSE-поток, 401 без auth, ошибка при невалидной коллекции |
| **Sessions** | TC-API-040–047 | CRUD сессий, экспорт JSON/Markdown, 404 |
| **Git Sync** | TC-API-050–053 | Валидация URL, CRUD репозиториев, 404, 401 |

#### Запуск API-тестов

```bash
# Внутри контейнера frontend-tests — только API-тесты
npx playwright test e2e/api-backend.spec.ts

# Запуск одной группы (например, Collections)
npx playwright test e2e/api-backend.spec.ts --grep "Collections"

# Запуск одного теста по ID
npx playwright test e2e/api-backend.spec.ts --grep "TC-API-020"
```

Через Makefile API-тесты входят в общий прогон `make test-e2e`.
...
2. **Параллельный запуск dev и test:** Тестовое окружение использует отдельные порты (например, `18000` для Chroma, `13000` для backend), поэтому dev-окружение (`make dev-up`) и test-окружение могут работать одновременно.
