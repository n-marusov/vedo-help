...
The system uses a Docker Compose microservice architecture. The **backend** is the orchestrator — it accepts user requests, validates KeyCloak JWTs, stores metadata in PostgreSQL, retrieves vectors from Chroma, calls RouterAI-compatible embedding/LLM APIs, and streams Server-Sent Events (SSE) responses. All service-to-service calls inside Compose use Docker service names and container ports.
...
graph TB
    ...
    subgraph "Docker Network"
        ...
        PG[(PostgreSQL<br/>App + KeyCloak data)]
        OTEL[OTel Collector]
    end

    subgraph External
        RAI[RouterAI / OpenAI-compatible APIs]
    end
    ...
    F -- "API /api/*" --> BE
    F -- "Token exchange" --> K
    BE -- "JWKS / JWT validation" --> K
    BE -- "sqlx" --> PG
    BE -- "Chroma API" --> CDB
    BE -- "/v1/embeddings + /chat/completions" --> RAI
    BE -- "OTLP" --> OTEL
    K --> PG
```
...
├── lib.rs               # Re-exports
├── modules/
│   ├── documents/       # Upload, parsing, chunking, reload
│   ├── collections/     # Collection CRUD, stats, chunk search
│   ├── query/           # Advanced RAG pipeline, Q&A, debug data
│   ├── conversations/   # Chat sessions, messages, export/edit/delete
│   ├── auth/            # Auth endpoints (me, logout), UserContext
│   ├── git_sync/        # Git repository sync (clone, pull, parse, index)
│   ├── web_crawl/       # BFS website crawl jobs, pages, SSE progress
│   ├── audit/           # Admin audit log storage and API
...
└── shared/
    ...
    ├── embedding_client.rs  # RouterAI-compatible embedding client
...
### 2. RouterAI-compatible Embeddings

Embeddings are generated by the backend through an OpenAI-compatible `/v1/embeddings` API. `EMBEDDING_API_KEY` and `EMBEDDING_BASE_URL` can override the LLM credentials/base URL, and `EMBEDDING_CACHE_SIZE` controls the local in-memory LRU cache.

There is no separate Python embedding container in the current implementation.
...
PostgreSQL 16 stores application metadata (collections, documents, chunks, sessions, messages, Git repositories, web crawl jobs/pages, settings, and audit events) and KeyCloak data in separate databases initialized by `scripts/init-db.sh`.

### 5. KeyCloak (OIDC/OAuth2 Identity Provider)
...
Single-page application with five routes and an admin workspace:

- **Chat view** (`/`) — streaming Q&A interface with message history
- **Admin view** (`/admin`) — collections, document upload, Git sync, web crawl, debug, health, statistics, and settings
...
- **Avatar Preview view** (`/ui-preview/avatar`) — UI component playground
...
│   ├── HealthStatus.vue   # Deep healthcheck display
│   ├── StatsPanel.vue     # Collection statistics
│   ├── ChunkBrowser.vue   # Chunk text/semantic search
...
│   └── WebCrawlManager.vue # Website crawl jobs
├── stores/
...
When an administrator uploads a document, the backend orchestrates validation, parsing, chunking, embedding, and indexing. Chunk/document metadata is stored in PostgreSQL; vector embeddings are stored in Chroma.
...
sequenceDiagram
    ...
    participant E as RouterAI Embeddings
    ...
    loop For each batch of chunks
        B->>E: POST /v1/embeddings (chunk texts)
...
7. Backend sends chunk texts in batches to the embedding API, then stores the resulting vectors in Chroma with document metadata for filtered search
