...
const isAuthenticated = ref(!!getApiKey());
...
export async function handleCallback(): Promise<string> {
  ...
  if (!verifier) {
    throw new Error('PKCE code verifier not found. Please start the login flow again.');
  }
  ...
  if (!tokenResponse.ok) {
    ...
    console.error('[OidcAuth] Token exchange failed:', tokenResponse.status, errorBody);
 ...
 *
 * On success, updates the stored access + refresh tokens, calls `setApiKey`,
...
async function refreshAccessToken(): Promise<string | null> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.warn('[OidcAuth] Token refresh failed:', response.status, errorBody);
