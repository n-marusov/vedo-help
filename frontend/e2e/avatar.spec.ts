...
test.describe("UserAvatar component", () => {
	...
	test("TC-AVATAR-001: renders user avatar with person SVG and accessible label", async ({
		...
		await expect(
			avatar.locator('[data-testid="user-avatar-icon"]'),
		).toBeVisible();
		await expect(
			avatar.locator('[data-testid="assistant-avatar-icon"]'),
		).toHaveCount(0);
	});

	test("TC-AVATAR-002: renders assistant avatar with branded V SVG", async ({
		page,
	}) => {
	...
	test("TC-AVATAR-003: uses role-specific token colors", async ({ page }) => {
		...
		const userColor = await userAvatar.evaluate(
	...
	test("TC-AVATAR-004: maps size variants from the avatar token", async ({
		page,
	}) => {
	...
	test("TC-AVATAR-005: keeps avatar implementation emoji-free", async ({
		page,
	}) => {
