...
test.describe("Auth Regression: Collection API calls", () => {
	...
	test("TC-AUTH-REG-002: API 401 on collections does not crash the app", async ({
		page,
	}) => {
		await page.addInitScript((token) => {
			localStorage.setItem("vedo_auth_token", token);
...
test.describe("Auth Regression: Token handling", () => {
	test("TC-AUTH-REG-010: missing token redirects to login", async ({
		page,
	}) => {
	...
	test("TC-AUTH-REG-011: expired token redirects to login", async ({
		page,
	}) => {
		const expiredToken = [
	...
	test("TC-AUTH-REG-012: all API routes require auth header", async ({
		page,
	}) => {
		const endpoints = [
		...
		for (const ep of endpoints) {
			...
			expect(
				response.status(),
				`Expected 401 for ${ep.method} ${ep.path} without auth`,
			).toBe(401);
		}
	});

	test("TC-AUTH-REG-013: all API routes return 401 with invalid token", async ({
		page,
	}) => {
		const endpoints = [
			{ method: "GET", path: "/api/collections" },
		...
		for (const ep of endpoints) {
			...
			expect(
				response.status(),
				`Expected 401 for ${ep.method} ${ep.path} with invalid token`,
			).toBe(401);
		}
	});
});
