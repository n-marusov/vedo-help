...
test.describe("Auth Regression: real backend API calls", () => {
	test("TC-AUTH-REG-001: collection API accepts a real KeyCloak JWT", async ({
		page,
		request,
	}) => {
	...
	test("TC-AUTH-REG-002: app renders with real backend collections", async ({
		page,
		request,
	}) => {
...
test.describe("Auth Regression: token handling", () => {
	...
	test("TC-AUTH-REG-012: API routes require auth header", async ({
		request,
	}) => {
		const endpoints = [
	...
	test("TC-AUTH-REG-013: API routes return 401 with invalid token", async ({
		request,
	}) => {
		const endpoints = [
			{ method: "GET", path: "/api/collections" },
