...
test.describe('Theme Switching (all pages)', () => {
  ...
  test.describe('Login Page', () => {
    ...
    test('TC-THEME-LOGIN-009: OAuth button border colors update on theme switch', async ({ page }) => {
      ...
      const lightBorder = await oauthBtn.evaluate((el) => getComputedStyle(el).borderColor);
  ...
  test.describe('Admin Page', () => {
    ...
    test('TC-THEME-ADMIN-005: admin auth card background updates on theme switch', async ({ page }) => {
      ...
      const darkBg = await authCard.evaluate((el) => getComputedStyle(el).backgroundColor);
      expect(darkBg).toBe('rgb(22, 22, 46)'); // --color-card dark: #16162e

      await toggle.click();

      const lightBg = await authCard.evaluate((el) => getComputedStyle(el).backgroundColor);
    ...
    test('TC-THEME-ADMIN-006: admin panel text colors update on theme switch', async ({ page }) => {
      ...
      const darkColor = await authTitle.evaluate((el) => getComputedStyle(el).color);

      await toggle.click();

      const lightColor = await authTitle.evaluate((el) => getComputedStyle(el).color);
