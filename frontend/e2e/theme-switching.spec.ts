...
test.describe("Theme Switching (all pages)", () => {
	...
	test.describe("Login Page", () => {
		test("TC-THEME-LOGIN-001: login page defaults to dark theme", async ({
			page,
		}) => {
		...
		test("TC-THEME-LOGIN-002: theme toggle is visible on login page", async ({
			page,
		}) => {
			await page.goto("/login");
		...
		test("TC-THEME-LOGIN-004: theme toggle shows sun icon in dark theme", async ({
			page,
		}) => {
			await page.goto("/login");
			const toggle = page.locator('[data-testid="theme-toggle"]');
		...
		test("TC-THEME-LOGIN-007: login card background changes on theme switch", async ({
			page,
		}) => {
			await page.goto("/login");
		...
		test("TC-THEME-LOGIN-008: login page text colors change on theme switch", async ({
			page,
		}) => {
			await page.goto("/login");
			const toggle = page.locator('[data-testid="theme-toggle"]');
			...
			const darkColor = await title.evaluate(
		...
		test("TC-THEME-LOGIN-009: OAuth button border colors update on theme switch", async ({
			...
			const darkBorder = await oauthBtn.evaluate(
	...
	test.describe("Chat Page", () => {
		...
		test("TC-THEME-CHAT-002: theme toggle is visible on chat page header", async ({
			page,
		}) => {
			await page.goto("/");
			const toggle = page.locator('[data-testid="theme-toggle"]');
		...
		test("TC-THEME-CHAT-005: chat toolbar background updates on theme switch", async ({
			page,
		}) => {
			await page.goto("/");
			const toggle = page.locator('[data-testid="theme-toggle"]');
			...
			const darkBg = await toolbar.evaluate(
				(el) => getComputedStyle(el).backgroundColor,
			);

			await toggle.click();

			const lightBg = await toolbar.evaluate(
				(el) => getComputedStyle(el).backgroundColor,
			);
		...
		test("TC-THEME-CHAT-006: welcome message colors update on theme switch", async ({
			page,
		}) => {
			await page.goto("/");
			const toggle = page.locator('[data-testid="theme-toggle"]');
			...
			const darkWelcomeColor = await welcome.evaluate(
				(el) => getComputedStyle(el).color,
			);

			await toggle.click();

			const lightWelcomeColor = await welcome.evaluate(
				(el) => getComputedStyle(el).color,
			);
			expect(darkWelcomeColor).not.toBe(lightWelcomeColor);
		});

		test("TC-THEME-CHAT-007: chat page composer area reflects theme change", async ({
			page,
		}) => {
			await page.goto("/");
			const toggle = page.locator('[data-testid="theme-toggle"]');
			...
			const darkBg = await composer.evaluate(
				(el) => getComputedStyle(el).backgroundColor,
			);

			await toggle.click();

			const lightBg = await composer.evaluate(
				(el) => getComputedStyle(el).backgroundColor,
			);
			expect(darkBg).not.toBe(lightBg);
		});
	...
	test.describe("Admin Page", () => {
		...
		test("TC-THEME-ADMIN-002: theme toggle is visible on admin page header", async ({
			page,
		}) => {
			await page.goto("/admin");
			const toggle = page.locator('[data-testid="theme-toggle"]');
	...
	test.describe("Theme Persistence", () => {
		...
		test("TC-THEME-PERSIST-003: theme preference is saved to localStorage", async ({
			page,
		}) => {
			await page.goto("/login");
			await page.locator('[data-testid="theme-toggle"]').click();
			...
			const storedTheme = await page.evaluate(() =>
				localStorage.getItem("vedo_theme"),
			);
			expect(storedTheme).toBe("light");
		});

		test("TC-THEME-PERSIST-004: theme persists across page reload", async ({
			page,
		}) => {
		...
		test("TC-THEME-PERSIST-005: dark theme saves to localStorage when toggled back", async ({
			...
			await toggle.click();
			expect(
				await page.evaluate(() => localStorage.getItem("vedo_theme")),
			).toBe("light");
	...
	test.describe("Visual Token Values", () => {
		test("TC-THEME-TOKEN-001: all core tokens switch correctly on login page", async ({
			page,
		}) => {
			await page.goto("/login");
			const toggle = page.locator('[data-testid="theme-toggle"]');
		...
		test("TC-THEME-TOKEN-002: theme toggle uses ToggleButton design tokens", async ({
			page,
		}) => {
			await page.goto("/login");
			const toggle = page.locator('[data-testid="theme-toggle"]');
			...
			const bg = await toggle.evaluate(
				(el) => getComputedStyle(el).backgroundColor,
			);
		...
		test("TC-THEME-TOKEN-003: primary color remains unchanged across themes", async ({
			page,
		}) => {
			await page.goto("/login");
			const toggle = page.locator('[data-testid="theme-toggle"]');
	...
	test.describe("Accessibility", () => {
		test("TC-THEME-A11Y-001: theme toggle has accessible label", async ({
			page,
		}) => {
			await page.goto("/login");
			const toggle = page.locator('[data-testid="theme-toggle"]');
			await expect(toggle).toHaveAttribute(
				"aria-label",
				/toggle|theme|switch/i,
			);
		});

		test("TC-THEME-A11Y-002: theme toggle is keyboard-focusable", async ({
			page,
		}) => {
			await page.goto("/login");
			const toggle = page.locator('[data-testid="theme-toggle"]');
			...
			const tabIndex = await toggle.evaluate((el) =>
		...
		test("TC-THEME-A11Y-003: theme toggle responds to keyboard (Enter/Space)", async ({
			page,
		}) => {
			await page.goto("/login");
			const toggle = page.locator('[data-testid="theme-toggle"]');
	...
	test.describe("Edge Cases", () => {
		...
		test("TC-THEME-EDGE-002: invalid localStorage value defaults to dark theme", async ({
			...
			await page.evaluate(() =>
				localStorage.setItem("vedo_theme", "invalid_value"),
			);
		...
		test("TC-THEME-EDGE-003: removing localStorage theme falls back to dark", async ({
			page,
		}) => {
