...
test.describe("Login Page", () => {
	test("TC-LOGIN-001: login page is accessible at /login route", async ({
		page,
	}) => {
	...
	test("TC-LOGIN-002: login page displays app name and title", async ({
		page,
	}) => {
		await page.goto("/login");
	...
	test("TC-LOGIN-003: login page renders all 5 provider buttons", async ({
		page,
	}) => {
		await page.goto("/login");
	...
	test("TC-LOGIN-004: each provider button displays its label text", async ({
		page,
	}) => {
		await page.goto("/login");
	...
	test("TC-LOGIN-006: provider buttons are clickable and show loading state", async ({
		page,
	}) => {
		await page.goto("/login");
	...
	test("TC-LOGIN-009: callback page shows error on missing PKCE data and redirects back", async ({
		...
		await expect(errorMessage).toContainText(
			/PKCE|state|verifier|start the login flow/i,
		);
	...
	test("TC-LOGIN-010: callback page shows error state for failed auth", async ({
		page,
	}) => {
		await page.goto(
			"/callback?error=access_denied&error_description=Access+denied",
		);
		const callbackPage = page.locator('[data-testid="callback-page"]');
		await expect(callbackPage).toBeVisible({ timeout: 5000 });
	...
	test("TC-LOGIN-011: callback page shows error on missing auth code", async ({
		page,
	}) => {
	...
	test("TC-LOGIN-012: callback page shows error on state mismatch", async ({
		page,
	}) => {
		...
		await page.evaluate(() => {
			...
			localStorage.setItem(
				"vedo_pkce_verifier",
				"mock-verifier-xxxxxxxxxxxxxxxxxxxxxxxxxxxxx",
			);
		});
	...
	test("TC-LOGIN-013: login page subtitle text is correct", async ({
		page,
	}) => {
		await page.goto("/login");
	...
	test("TC-LOGIN-016: login page background uses CSS custom properties", async ({
		page,
	}) => {
		await page.goto("/login");
...
test.describe("Auth Guard & Session", () => {
	...
	test("TC-AUTH-003: login page is accessible without auth", async ({
		page,
	}) => {
		await page.goto("/login");
		const loginPage = page.locator('[data-testid="login-page"]');
	...
	test("TC-AUTH-004: callback page is accessible without auth", async ({
		page,
	}) => {
		await page.goto("/callback");
		const callbackPage = page.locator('[data-testid="callback-page"]');
	...
	test("TC-AUTH-005: authenticated user can access chat page", async ({
		page,
	}) => {
	...
	test("TC-AUTH-007: invalid JWT token redirects to login", async ({
		page,
	}) => {
	...
	test("TC-AUTH-008: clearing localStorage logs out user after page reload", async ({
		page,
	}) => {
		const validToken = makeMockJwt({
			sub: "user-123",
