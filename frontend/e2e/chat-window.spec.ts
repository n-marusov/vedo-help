...
test.describe("ChatWindow Layout with real backend", () => {
	...
	test.beforeEach(async ({ page, request }) => {
		collection = await setupAuthAndCollection(
			page,
			request,
			`ChatWindow ${Date.now()}`,
		);
	...
	test("TC-CHAT-001: renders chat header with collection selector", async ({
		...
		await expect(
			page.locator('[data-testid="collection-select"]'),
		).toBeVisible();
	});

	test("TC-CHAT-002: collection selector shows real backend collections", async ({
		page,
	}) => {
		await page.goto("/");
	...
	test("TC-CHAT-004: input enables send when a real collection is selected", async ({
		page,
	}) => {
		await page.goto("/");
	...
	test("TC-CHAT-005: Shift+Enter inserts newline instead of sending", async ({
		page,
	}) => {
		await page.goto("/");
		await setActiveCollection(page, collection.id);
