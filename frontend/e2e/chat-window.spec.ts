...
test.describe("ChatWindow Layout", () => {
	...
	test("TC-CHAT-002: collection selector shows collections list", async ({
		page,
	}) => {
		await page.goto("/");
		const collectionSelect = page.locator('[data-testid="collection-select"]');
	...
	test("TC-CHAT-003: new chat button is visible in header", async ({
		page,
	}) => {
		await page.goto("/");
	...
	test("TC-CHAT-005: messages area is scrollable", async ({ page }) => {
		...
		const overflowY = await messagesArea.evaluate(
			(el) => getComputedStyle(el).overflowY,
		);
		expect(overflowY).toBe("auto");
	});

	test("TC-CHAT-006: shows welcome message when no messages exist", async ({
		page,
	}) => {
		await page.goto("/");
		const welcomeMsg = page.locator('[data-testid="welcome-message"]');
	...
	test("TC-CHAT-007: hides welcome message when messages exist", async ({
		page,
	}) => {
		await page.goto("/");
		const welcomeMsg = page.locator('[data-testid="welcome-message"]');
	...
	test.describe("Input Area", () => {
		...
		test("TC-CHAT-011: send button is disabled when input is empty", async ({
			page,
		}) => {
			await page.goto("/");
			const sendBtn = page.locator('[data-testid="btn-send"]');
		...
		test("TC-CHAT-014: Shift+Enter inserts newline instead of sending", async ({
			page,
		}) => {
			await page.goto("/");
			const input = page.locator('[data-testid="chat-input"]');
		...
		test("TC-CHAT-015: cancel button appears during streaming", async ({
			page,
		}) => {
			await page.goto("/");
	...
	test.describe("Message Animations (Task 3.3)", () => {
		...
		test("TC-ANIM-002: message animation has reasonable duration", async ({
			page,
		}) => {
			await page.goto("/");
			const message = page.locator('[data-testid^="message-"]').first();
		...
		test("TC-ANIM-003: scroll to bottom on new message works", async ({
			page,
		}) => {
			await page.goto("/");
			const messagesArea = page.locator('[data-testid="messages-area"]');
			...
			if (scrollHeight > clientHeight) {
				expect(scrollTop).toBeGreaterThanOrEqual(
