...
test.describe("Document Re-indexing: Upload → Reload → Delete", () => {
	...
	test("TC-REINDEX-001: upload document then query shows it as a source", async ({
		page,
	}) => {
	...
	test("TC-REINDEX-002: reload document replaces old content in sources", async ({
		page,
	}) => {
	...
	test("TC-REINDEX-003: deleted document is absent from query sources", async ({
		page,
	}) => {
		...
		const deletedDocInSources = await sourceItems
			...
			.evaluateAll((elements) =>
				elements.some((el) => el.textContent?.includes("delete-me.md")),
			);
		...
		const otherDocInSources = await sourceItems
			.locator('[data-testid="source-document"]')
			.evaluateAll((elements) =>
				elements.some((el) => el.textContent?.includes("other-doc.md")),
			);
