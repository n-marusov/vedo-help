...
test.describe("RAG Flow: real backend upload → query → sources", () => {
	test("TC-RAG-001: admin page renders with real backend data", async ({
		page,
		request,
	}) => {
		...
		await expect(
			page.locator('[data-testid="auth-section"]'),
		).not.toBeVisible();
		await expect(page.locator(".admin-panel")).toBeVisible();
	});

	test("TC-RAG-002: upload markdown document through UI to backend", async ({
		page,
		request,
	}) => {
	...
	test("TC-RAG-003: query real backend and render streaming response", async ({
		...
		).toContainText(/backend answer|Sources/i, {
			timeout: 30000,
		});
	});
});
