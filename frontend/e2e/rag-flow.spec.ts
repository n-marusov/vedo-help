...
test.describe('RAG Flow: Upload → Query → Sources', () => {
  ...
  test('TC-RAG-001: admin page renders with admin panel when authenticated', async ({ page }) => {
  ...
  test('TC-RAG-002: upload document via admin panel (mocked)', async ({ page }) => {
  ...
  test('TC-RAG-003: send query and verify streaming response', async ({ page }) => {
    await page.addInitScript((token) => {
      localStorage.setItem('vedo_auth_token', token);
    }, VALID_TOKEN);
