...
test.describe("Git Sync: real backend repository management", () => {
	test("TC-GIT-001: register new git repo through UI → row appears", async ({
		page,
		request,
	}) => {
		...
		await expect(
			repoRow.locator('[data-testid="git-repo-status"]'),
		).toContainText(/idle/i);
	});

	test("TC-GIT-002: backend rejects invalid repo URL", async ({
		request,
		page,
	}) => {
		const collection = await setupAuthAndCollection(
	...
	test("TC-GIT-003: create and delete repo through real backend API", async ({
		request,
		page,
	}) => {
		const collection = await setupAuthAndCollection(
		...
		const repos = await apiRequest<Array<{ id: string }>>(
			request,
			"GET",
			"/api/git-sync/repos",
		);
