...
test.describe('Git Sync: Repository Management', () => {
  ...
  test('TC-GIT-002: trigger sync and observe status transitions', async ({ page }) => {
    ...
    await page.route('**/api/git-sync/repos/repo-1/sync', async (route, request) => {
      syncRequested = true;
  ...
  test('TC-GIT-003: delete repo → confirm dialog → row removed', async ({ page }) => {
    ...
    // Click confirm
    const confirmBtn = confirmDialog.locator('[data-testid="btn-dialog-confirm"]');
  ...
  test('TC-GIT-004: form validation errors — empty submit and invalid URL', async ({ page }) => {
  ...
  test('TC-GIT-005: sync error state — broken URL shows error badge', async ({ page }) => {
  ...
  test('TC-GIT-006: list shows repos for the selected collection only', async ({ page }) => {
    const multipleRepos = [
      {
        id: 'repo-1',
  ...
  test('TC-GIT-007: unauthenticated access returns 401 redirect', async ({ page }) => {
  ...
  test('TC-GIT-008: delete dialog cancel → row stays, no DELETE sent', async ({ page }) => {
    let deleteCalled = false;
    ...
    const cancelBtn = confirmDialog.locator('[data-testid="btn-dialog-cancel"]');
  ...
  test('TC-GIT-009: empty repos list shows zero-state message', async ({ page }) => {
    // Mock empty repos list
    await page.route('**/api/git-sync/repos', async (route, request) => {
      if (request.method() === 'GET') {
        await route.fulfill({
